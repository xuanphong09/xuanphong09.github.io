...
const musicOptions = [
    ...
    {
        value: "./music/happybirthday.mp3",
        label: "Happy Birthday (Phiên bản 2)",
    },
    {
        value: "./music/happybirthday_domixi.mp3",
        label: "Happy Birthday (Độ Mixi)",
    },
    {
        value: "./music/happybirtday_uia.mp3",
        label: "Happy Birthday (Mèo UIA)",
    },
    ...
    {
        value: "./music/dunglamtraitimanhdau.mp3",
        label: "Đừng làm trái tim anh đau",
    },
    ...
    {
        value: "./music/emlacogiaovungcaoremix.mp3",
        label: "Em là cô giáo vùng cao (Remix)",
    },
    ...
    {
        value: "./music/minhcungnhaudongbang.mp3",
        label: "Mình cùng nhau đóng băng",
    },
    {
        value: "./music/neunhucohaitraitim.mp3",
        label: "Nếu như có hai trái tim",
    },
    ...
    {
        value: "./music/trentinhbanduoitinhyeu.mp3",
        label: "Trên tình bạn dưới tình yêu",
    },
...
const gifOptions = [
    ...
    {
        value: "./gif/Happy-Birthday-GIF-by-BREAD-TR-unscreen.gif",
        label: "Gif3",
    },
...
function setMusicPreviewState({ message, isPlaying }) {
    if (musicPreviewButton) {
        musicPreviewButton.textContent = isPlaying
            ? "⏸ Dừng nghe thử"
            : "▶ Nghe thử";
        if (isPlaying) {
...
function populateModal() {
    ...
    if (isSaveCheckbox) {
        const savedState =
            window.lastIsSaveState !== undefined
                ? window.lastIsSaveState
                : settings.isSave;
    ...
    sequenceColorInput.addEventListener("input", () => {
        ...
        if (sequenceColorPreview) {
            sequenceColorPreview.style.backgroundColor =
                sequenceColorInput.value;
        }
    });
...
function handleColorThemeChange(selectedTheme) {
    ...
    const sequenceColorSection = document.getElementById(
        "sequenceColorSection"
    );
...
function saveFormDataToSettings() {
    try {
        ...
        const sequenceColorInput = document.getElementById("sequenceColor");
        if (sequenceColorInput)
            settings.sequenceColor = sequenceColorInput.value;
...
function updateExpireDateDisplay() {
    ...
    if (isSaveCheckbox && expireDateElement && expireContainer) {
        if (isSaveCheckbox.checked) {
            ...
            expireContainer.innerHTML = `⏳ ${t(
                "expireText"
            )} <b id="expireDate" style="color: #4caf50;">${t(
                "saveForever"
            )}</b>`;
        } else {
            ...
            expireContainer.innerHTML = `⏳ ${t(
                "expireText"
            )} <b id="expireDate">${t("thirtyDays")}</b>`;
        }
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (websiteId) {
        const googleAuthContainer = document.getElementById(
            "googleAuthContainer"
        );
...
async function loadWebsiteFromServer() {
    ...
    try {
        ...
        const result = await window.birthdayAPI.getBirthdayWebsiteByWebsiteId(
            websiteId
        );
...
// Cập nhật event listener cho apply settings để sử dụng hàm reset chung
function runBirthdayApp() {
    // 1. Kiểm tra enableBook trước
    const enableBookSelect = document.getElementById("enableBook");
    // Nếu input chưa load kịp (lúc auto start), lấy từ settings gốc
    const isBookEnabled = enableBookSelect 
        ? (enableBookSelect.value === "true") 
        : settings.enableBook;

    // 2. Logic kiểm tra sách (chỉ alert khi người dùng bấm nút, không alert lúc tự động chạy)
    // Chúng ta dùng biến event để biết đây là auto run hay click (mẹo nhỏ)
    const isAutoRun = !event || !event.target; 

    if (isBookEnabled && !isAutoRun) {
        ...
        // Kiểm tra số trang chẵn lẻ
        const totalPages = settings.pages.length;
        if (totalPages > 1 && totalPages % 2 === 0) {
            alert(`❌ Cấu trúc trang không hợp lệ! Hiện tại có ${totalPages} trang. Vui lòng thêm hoặc xóa 1 trang.`);
            return;
        }
    }

    // 3. Cập nhật settings cơ bản từ form (Nếu form có dữ liệu)
    const musicInput = document.getElementById("backgroundMusic");
    if (musicInput) settings.music = musicInput.value || "./music/happy-birthday.mp3";

    const countdownInput = document.getElementById("countdownTime");
    if (countdownInput) settings.countdown = parseInt(countdownInput.value) || 3;

    const matrixTextInput = document.getElementById("matrixText");
    if (matrixTextInput) settings.matrixText = matrixTextInput.value || "HAPPYBIRTHDAY";

    // ... (Các phần lấy màu giữ nguyên như code cũ của bạn) ...
    // Để code gọn mình viết tắt đoạn này, bạn giữ nguyên phần lấy value color nhé
    if(document.getElementById("matrixColor1")) settings.matrixColor1 = document.getElementById("matrixColor1").value;
    if(document.getElementById("matrixColor2")) settings.matrixColor2 = document.getElementById("matrixColor2").value;
    if(document.getElementById("sequenceText")) settings.sequence = document.getElementById("sequenceText").value;
    if(document.getElementById("sequenceColor")) settings.sequenceColor = document.getElementById("sequenceColor").value;
    if(document.getElementById("giftImage")) settings.gift = document.getElementById("giftImage").value || "./gif/Cat Love GIF by KIKI.gif";
    ...
    const isSaveInput = document.getElementById("isSave");
    settings.isSave = isSaveInput?.checked || false;

    // Cập nhật trạng thái global
    window.lastIsSaveState = settings.isSave;


    // --- 4. PHẦN QUAN TRỌNG NHẤT: XỬ LÝ ẢNH ---
    // Tạo một mảng mới hoàn toàn để tránh lỗi tham chiếu
    const updatedPages = [];

    if (settings.pages && settings.pages.length > 0) {
        settings.pages.forEach((oldPage, index) => {
            const fileInput = document.getElementById(`pageImage${index}`);
            const contentInput = document.getElementById(`pageContent${index}`);

            const newPageData = { ...oldPage }; // Copy dữ liệu cũ

            // LOGIC QUYẾT ĐỊNH ẢNH:
            // 1. Nếu người dùng chọn file mới -> Dùng file mới (Blob URL)
            // 2. Nếu KHÔNG chọn file -> Giữ nguyên ảnh cũ (có thể là link "./image/..." hoặc blob cũ)
            
            if (fileInput && fileInput.files && fileInput.files.length > 0) {
                // Người dùng có chọn file mới
                newPageData.image = URL.createObjectURL(fileInput.files[0]);
                // Lưu file vào để lần sau populateModal hiển thị lại preview đúng
                newPageData.selectedFile = fileInput.files[0]; 
            } else {
                // Không chọn file mới -> Giữ nguyên newPageData.image cũ
                // (Lúc khởi tạo nó là "./image/1.png", nên nó sẽ giữ nguyên link này)
            }

            // Lấy nội dung text
            if (contentInput) {
                newPageData.content = contentInput.value;
            }

            updatedPages.push(newPageData);
        });
        
        // Cập nhật lại vào settings chính
        settings.pages = updatedPages;
    }

    // 5. Các hàm reset và khởi chạy
    if (window.pricingCalculator) {
        window.pricingCalculator.updateFromSettings(settings);
    }
...
applySettingsButton.addEventListener("click", runBirthdayApp);
/* --- ĐOẠN CODE KHỞI CHẠY KÈM ĐẾM NGƯỢC --- */
window.addEventListener("load", () => {
    // Delay nhỏ để đảm bảo các thẻ input trong modal đã được sinh ra (populateModal)
    // Nếu không có populateModal chạy trước, getElementById sẽ null
    setTimeout(() => {
        // console.log("Auto starting...");
        
        // QUAN TRỌNG: Phải gọi populateModal() một lần để tạo ra các thẻ input id="pageImage0"...
        // Nếu không có thẻ input, hàm runBirthdayApp sẽ không tìm thấy element để check
        // if (typeof populateModal === "function") {
        //     populateModal(); 
        //     // Ẩn modal đi vì populateModal mặc định có thể hiện preview
        //     if(settingsModal) settingsModal.style.display = "none";
        // }
        ...
        // // Xử lý nhạc tự động (fix lỗi autoplay)
        // const audio = document.getElementById("birthdayAudio");
        // if(audio) {
        //     audio.src = settings.music; // Đảm bảo src đúng
        //     audio.play().catch(() => {
